...
class Dot(pygame.sprite.Sprite):
    ...
    def respawn(self, color, radius=5, offset=5):
        return Dot(
            self.rect.x,
            self.rect.y,
            self.WIDTH,
        ...
        for i in range(self.T):
            ...
            for guy in self.bacteria_infected_container:
                if guy.killswitch_on and guy.cycles_to_fate == 0:
                    for _ in range(
                            int(np.random.uniform(self.virions_count[0], self.virions_count[1]))):  # Spawn red dots
...
if __name__ == '__main__':
    covid = Simulation()
    covid.n_susceptible = 75
    covid.n_quarantined = 0
    covid.n_infected = 2
    covid.T = 1500
    covid.cycle_to_fate = 50
    covid.mortality_rate = 0.14
    covid.virions_count = (6, 12)
